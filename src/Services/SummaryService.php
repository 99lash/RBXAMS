...
class SummaryService
{
	...
	public function __construct(
		SummaryRepository $summaryRepo = null,
	...
	public function recomputeDailySummary(string $userId, string $date): void
	{
		// Delete existing summary for the day to ensure a clean recomputation
		$this->summaryRepo->deleteByDate($userId, $date);

		// Ensure a fresh summary entry exists for the day
		$this->ensureSummaryExists($userId, $date);

		// Get all active BUY and SELL transactions for the given day
		$transactions = $this->transactionRepo->findActiveTransactionsByDate($userId, $date);
		...
		foreach ($transactions as $transaction) {
			...
			if ($transaction['transaction_type'] === 'BUY') {
				$this->updateSummaryOnBuy($userId, $account);
			} elseif ($transaction['transaction_type'] === 'SELL') {
				$this->updateSummaryOnSell($userId, $account);
			}
		}
	}
}
